...
describe("type", () => {
  ...
  it("keeps diagram labels at 12.8px+ even at the diagram's narrowest", async () => {
    const { confidenceGrid, checkScale, signoffTimeline, checklistGrid, fixFirstGrid, controlGrid, overlapPlot, patternTimeline, gainsFigure, planGrid } = await import("../lesson-core.js");
    const { getLesson } = await import("../course.js");
    const eight = Array.from({ length: 8 }, (_, i) => ({ id: `c${i}`, catches: [] }));
    const viewBoxWidth = Math.max(...[confidenceGrid([]), checkScale([]), signoffTimeline("2026-01-01"), checklistGrid([], eight), fixFirstGrid([]), controlGrid([]), overlapPlot(getLesson(9).stages[1].overlap), patternTimeline(getLesson(10).stages[0].tracks.educators.thread.moments), gainsFigure(getLesson(11).stages[0].tracks.educators.chat), planGrid([], getLesson(12).stages[1].steps)].map((svg) => Number(svg.match(/viewBox="0 0 ([\d.]+)/)[1])));
