...
function referencedClaims() {
  ...
  for (const lesson of COURSE.lessons.filter((l) => l.ready)) {
    for (const stage of lesson.stages) {
      ...
      for (const track of Object.values(stage.tracks ?? {})) refs.add((track.passage ?? track.pair ?? track.signoffs ?? track.classify ?? track.screen ?? track.thread ?? track.chat).claim);
    }
  }
  return refs;
}
